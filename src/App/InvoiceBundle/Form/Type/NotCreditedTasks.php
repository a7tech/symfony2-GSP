...
class NotCreditedTasks extends TaskSelectType
{
    public function buildView(FormView $view, FormInterface $form, array $options)
    {
        $css_class = 'not-credited-tasks';
    ...
    public function setDefaultOptions(OptionsResolverInterface $resolver)
    {
        ...
        $resolver->setDefaults([
            ...
            'choices' => function(Options $options) use ($class) {
                $this->tasks = $this->entityManager->getRepository($class)->getNotCreditedTasks($options['project']);
                return $this->tasks;
            }
        ]);
    ...
    {
        return 'invoice_not_credited_tasks';
    }
    ...
    {
        return [
            'net' => $task->getType() == Task::TYPE_ADJUSTMENT ? $task->getNetPrice(true) : null
        ];
    }
